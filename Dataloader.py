...
class NILMDataloader():
    ...
    def _get_loader(self, dataset):
        dataloader = data_utils.DataLoader(
            dataset, batch_size=self.batch_size, shuffle=False, pin_memory=True, drop_last=False)
        return dataloader


class NILMDataset(data_utils.Dataset):
    def __init__(self, x, y, status, index, window_size=480, stride=30):
        self.x = x
        self.y = y
        self.status = status
    ...
    def __getitem__(self, index):
        ...
        time = self.padding_seqs(self.index[start_index: end_index])
        # float_dt = np.datetime64.astype(time.values, np.float64)
        return torch.tensor(x), torch.tensor(y), torch.tensor(status), torch.tensor(time.values.astype(np.float64))
...
class BERTDataset(data_utils.Dataset):
    def __init__(self, x, y, status, index, window_size=480, stride=30, mask_prob=0.2):
        self.x = x
        self.y = y
        self.status = status
        self.window_size = window_size
    ...
    def __getitem__(self, index):
        ...
        for i in range(len(x)):
            ...
            if prob < self.mask_prob:
                ...
                labels.append(temp)
                on_offs.append(temp)
            times.append(time.values[i])
        return torch.tensor(tokens), torch.tensor(labels), torch.tensor(on_offs), torch.tensor(list(map(float, times)))
