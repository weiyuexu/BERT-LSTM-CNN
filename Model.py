...
class MST(nn.Module):
    def __init__(self, args):
        ...
        self.lstmlayer = nn.LSTM(256, 256)
        self.conv1 = nn.Conv1d(240, 240, kernel_size=1, stride=1, bias=False)
        ...
        self.classifier = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv1d(in_channels=30, out_channels=471, kernel_size=8),
            nn.ReLU(),
            nn.Conv1d(in_channels=471, out_channels=40, kernel_size=6),
            nn.ReLU(),
            nn.Conv1d(in_channels=40, out_channels=459, kernel_size=5),
            nn.ReLU(),
            nn.Conv1d(in_channels=459, out_channels=50, kernel_size=5),
            nn.ReLU(),
            nn.Conv1d(in_channels=50, out_channels=50, kernel_size=5),
            nn.ReLU(),
            nn.Flatten(),
            nn.ReLU(),
            nn.Linear(22350, 1024),
            nn.ReLU(),
            nn.Linear(1024, 480),
            nn.Sigmoid()
        )
    ...
    def forward(self, sequence):
        ...
        x, (hn, cn) = self.lstmlayer(x)
        x = self.conv1(x)
        x = self.deconv(x.permute(0, 2, 1)).permute(0, 2, 1)
        x = torch.tanh(self.linear1(x))
        x = self.linear2(x)

        y = self.classifier(sequence.unsqueeze(1))
        y = y.reshape(128, -1, 1)
